...
// fd/alive/close 统一存于共享 Session 控制块 那是唯一事实来源 本对象不重复存
...
class Connection {
public:
    std::shared_ptr<Session> sess_;     // 共享控制块 fd/alive/close/身份的唯一来源
    ...
    ~Connection() {
        ...
        if (this->sess_ && this->sess_->fd_ >= 0) {
...
private:
    void set_nonblock() {
        ...
        fcntl(this->sess_->fd_, F_SETFL, flags | O_NONBLOCK);
    }
};
