...
// post 只投递到目标线程执行 不判当前线程 同线程直投由调用方自己分流
#pragma once
...
public:
    ...
    // 当前线程是否是本事件循环线程
    // loop 入口把本对象登记到线程局部 邮箱据此判断本地直投还是跨线程投递
    bool is_in_loop_thread() const;

    // 把回调投递到本循环所属线程执行 跨线程安全
    // 只入队并唤醒 不保证立即执行 也不判当前是否已在本 loop 线程
    // 需要同线程直接执行的调用方自行用 is_in_loop_thread 分流 如 Mailbox
